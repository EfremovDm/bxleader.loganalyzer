...
$obAdminListHelper->setTitle(Loc::getMessage('BM_LA_CRON_TITLE'));
...
$sDirectory    = Option::get(ADMIN_MODULE_NAME, 'cron_log_path');
$bRecursive    = Option::get(ADMIN_MODULE_NAME, 'cron_log_recursive') == 'Y';
$sFilePrefix   = Option::get(ADMIN_MODULE_NAME, 'cron_log_prefix');

// дата-провайдер

$obDataProvider = new DataProvider($sFileProvider, $bShowQueries);
$obDataProvider->setLogSettings('cron', $sDirectory, $bRecursive, $sFilePrefix);
...
$obAdminListHelper->setList($obDataProvider->getStrings(), $obDataProvider->getLinesCount(), array(
    ...
    'DATE' => function($val, $arRec) {
        ...
        if (!empty($val)) {
            ...
            $sResult = $obDateTime->format('d.m H:i:s');
        }
        return $sResult;
    },
